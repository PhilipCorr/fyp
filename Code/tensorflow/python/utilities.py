...
def load_mnist(dataset, digits, path, asbytes, selection, return_labels, return_indices):
    """
...
def concatenate_images(sample_set, orientation):
    long_image = np.concatenate(sample_set, orientation)
    return long_image

def image_to_xy(sample):
    non_zero = np.nonzero(sample)
    xy = [non_zero[0], non_zero[1]]
    return xy
