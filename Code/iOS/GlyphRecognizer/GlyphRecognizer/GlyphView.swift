        ...
        if let context = self.glyph?.managedObjectContext {
            ...
            case .changed:
                if let stroke =  self.stroke {
                    ...
                    touch.x = Double(gesture.location(in: self).x / max(self.bounds.size.width, self.bounds.size.height))
                    touch.y = Double(gesture.location(in: self).y / max(self.bounds.size.width, self.bounds.size.height))
    ...
    override func draw(_ rect: CGRect) {
        ...
        if let strokes = self.glyph?.strokes?.array as? [Stroke] {
            for stroke in strokes {
                if let path = stroke.path(scale: Double(max(self.bounds.size.width, self.bounds.size.height)), interpolation: .Cubic) {
